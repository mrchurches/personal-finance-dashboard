...
export function getSpendingPatterns(database: SqliteDatabase, currency = "ARS"): SpendingPattern[] {
  const rows = database
    .prepare<{ currency: string }, CycleRow>(
      ...
      GROUP BY t.merchant_key, cycle
      ORDER BY t.merchant_key, cycle`,
    )
    .all({ currency });
  ...
  const byMerchant = new Map<string, CycleRow[]>();
  for (const row of rows) {
    const existing = byMerchant.get(row.merchantKey);
    if (existing === undefined) {
      byMerchant.set(row.merchantKey, [row]);
      continue;
    }
    existing.push(row);
  }
  ...
  for (const [merchantKey, cycleRows] of byMerchant) {
